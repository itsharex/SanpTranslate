...
**SnapTranslate** 是一款面向开发者和外语学习者的桌面截屏贴图翻译工具。框选屏幕任意区域，瞬间完成 OCR 文字识别 + AI 翻译，译文以贴图形式覆盖在原内容上，所见即所得。

**核心理念：** 一键框选 → 原位贴图 → 随截随译

> 截图贴图在原始位置 — 不弹窗、不跳转、不打断工作流。
...
| **文本翻译** | `Ctrl+Alt+M` 打开简洁文本翻译窗口，Ctrl+Enter 快捷翻译 |
| **本地 OCR** | 内置 Tesseract 离线引擎，自动识别截图中文字及段落坐标，无需联网 |
...
按 Ctrl+Alt+L         框选截图并贴图到原位
                         ↓
点击贴图下方「翻译」按钮    OCR + AI 翻译，译文覆盖在原文位置
                         ↓
点击「复制译文」         将译文复制到剪贴板
                         ↓
...
- **macOS**: macOS 12+，需 WebKit（系统自带）
- **Linux**: 支持 X11/Wayland，需 WebKitGTK
...
| OCR | Tesseract CLI（内置 `chi_sim` + `eng` 语言数据） |
